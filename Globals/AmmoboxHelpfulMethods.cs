...
namespace AmmoboxPlus
{
    class AmmoboxHelpfulMethods
    {
        ...
        // FIXME: THIS ENTIRE METHOD IS PROBABLY BROKEN AND NO LONGER UP TO DATE
        // THIS WILL BREAK.
        public static void explodeRocket(int shot, int projid, int projtype, Color color = default(Color), bool largeBlast = false, bool skipDamage = false)
        {
            ...
            {
                // FIXME: This was DustID.Fire before
                int id = Dust.NewDust(projectile.position, projectile.width, projectile.height, DustID.MinecartSpark, Scale: 1.5f);
                Main.dust[id].noGravity = true;
                Main.dust[id].velocity *= 7f;
                id = Dust.NewDust(projectile.position, projectile.width, projectile.height, DustID.MinecartSpark, Scale: 1.5f);
        ...
        public static void createBurst(int type, Vector2 initialPos, int owner, int damage, float radius = 10, float velocityMultiplier = 1.0f, int oneInX = 4, bool makeFriendly = false, int Count = 16)
        {
            ...
            for (done = 0; done < Count; ++done)
            {
                if ((i >= 0 && i <= 0.5) || (i >= 1 && i <= 1.5))
                {
                    ...
                    int num = Projectile.NewProjectile(Projectile.GetSource_None(), initialPos + new Vector2((float)x, (float)y), vel, type, damage, 0, owner);
        ...
        internal static bool processMinerOreDrop(NPC target, int oneInX = 100)
        {
            ...
            //  Spawn the item
            int index = Item.NewItem(Projectile.GetSource_None(), target.position, dropTable, WorldGen.genRand.Next(10, 30));
            if (Main.netMode == 1)
            {
        ...
        public static void blastArea(Vector2 position, int maxRange = 3, bool extraDrops = false)
        {
            ...
            for (int i = leftCornerX; i <= rightCornerX; i++)
            {
                for (int j = leftCornerY; j <= rightCornerY; j++)
                {
                    ...
                    if (dist < (double)maxRange && Main.tile[i, j] != null && Main.tile[i, j].WallType == 0)
                    {
                        breakWalls = true;
                        break;
                    }
            ...
            for (int i = leftCornerX; i < rightCornerX; i++)
            {
                for (int j = leftCornerY; j < rightCornerY; j++)
                {
                    ...
                    if (distance < maxRange)
                    {
                        ...
                        bool toBreak = true;
                        // FIXME: This had "tile.active()" check before
                        if (Main.tile[i, j] != null)
                        {
                            toBreak = true;
                            // FIXME: Hopefully these didn't change between versions Clueless
                            if (Main.tileDungeon[(int)Main.tile[i, j].TileType] ||
                                Main.tile[i, j].TileType == 88 ||
                                TileID.Sets.BasicChest[(int)Main.tile[i, j].TileType] ||
                        ...
                        if (toBreak)
                        {
                            for (int x = i - 1; x <= i + 1; x++)
                            {
                                ...
                                {
                                    if (Main.tile[x, y] != null && Main.tile[x, y].WallType > 0 && breakWalls)
                                    {
            ...
            if (posTypeExtraOreDictionary.Count > 0 && extraDrops)
            {
                foreach (var entry in posTypeExtraOreDictionary)
                {
                    ...
                    int id = Item.NewItem(Item.GetSource_None(), entry.Key * 16f, entry.Value, 1);
                    if (Main.netMode != 0)
                    {

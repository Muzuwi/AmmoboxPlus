    ...
    public class AmmoboxGlobalNPC : GlobalNPC
    {
        ...
        public override void OnKill(NPC npc)
        {
            if (npc.boss && !Main.expertMode)
            {
                if (Main.hardMode)
                {
                    int id = Item.NewItem(Item.GetSource_None(), npc.getRect(), Mod.Find<ModItem>("AmmoBoxPlus").Type);
                ...
                {
                    int id = Item.NewItem(Item.GetSource_None(), npc.getRect(), Mod.Find<ModItem>("AmmoBox").Type);
                    if (Main.netMode != NetmodeID.SinglePlayer)
                    {
                        NetMessage.SendData(MessageID.SyncItem, -1, -1, null, id);
                    }
            ...
            if (Main.rand.NextBool(10) && apExtraHeartTick > 0)
            {
                int id1 = Item.NewItem(Item.GetSource_None(), npc.getRect(), ItemID.Heart);
                int id2 = Item.NewItem(Item.GetSource_None(), npc.getRect(), ItemID.Heart);
                if (Main.netMode != NetmodeID.SinglePlayer)
                {
            ...
            if (Main.rand.NextBool(10) && apExtraManaTick > 0)
            {
                int id1 = Item.NewItem(Item.GetSource_None(), npc.getRect(), ItemID.Star);
                int id2 = Item.NewItem(Item.GetSource_None(), npc.getRect(), ItemID.Star);
                if (Main.netMode != NetmodeID.SinglePlayer)
                {
                    NetMessage.SendData(MessageID.SyncItem, -1, -1, null, id1);
                    NetMessage.SendData(MessageID.SyncItem, -1, -1, null, id2);
        ...
        public override bool CheckDead(NPC npc)
        {
            if (npc.GetGlobalNPC<AmmoboxGlobalNPC>().apStuck && npc.realLife != -1)
            {
                ...
                foreach (NPC n in Main.npc)
                {
                    ...
                    {
                        n.StrikeNPC(npc.CalculateHitInfo(npc.life, 1));
                    }
                    ++index;
                }
                return true;
            ...
            if (apDrugged && apDruggedTick == 0)
            {
                ...
                foreach (NPC n in Main.npc)
                {
                    ...
                    if (dist < reach && n.active && !n.friendly && (n.whoAmI != npc.whoAmI) && !n.GetGlobalNPC<AmmoboxGlobalNPC>().apDrugged && !n.dontTakeDamage)
                    {
                        ...
                        {
                            n.StrikeNPC(npc.CalculateHitInfo(npc.damage / 8, 1));
                        }
                        else
                        {
                            n.StrikeNPC(npc.CalculateHitInfo(npc.damage / 2, 1));
                        }
            ...
            if (apCactus)
            {
                ...
                foreach (NPC n in Main.npc)
                {
                    ...
                    if ((collision) && n.active && !n.friendly && (n.whoAmI != npc.whoAmI) && !n.GetGlobalNPC<AmmoboxGlobalNPC>().apCactus && n.GetGlobalNPC<AmmoboxGlobalNPC>().apCactusCooldown == 0 && !n.dontTakeDamage)
                    {
                        ...
                        {
                            n.StrikeNPC(npc.CalculateHitInfo(npc.damage / 4, 1));
                        }
                        else
                        {
                            n.StrikeNPC(npc.CalculateHitInfo(npc.damage / 4, 1));
